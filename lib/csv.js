...
function parseMoney(value, decimalSeparator = null) {
...
function inferMoneyDecimalSeparator(rows, columnIndex) {
  let dotEvidence = 0;
  let commaEvidence = 0;
  for (const row of rows.slice(0, 500)) {
    const raw = String(row?.[columnIndex] ?? '').replace(/R\$/gi, '').replace(/\s/g, '');
    const lastDot = raw.lastIndexOf('.');
    const lastComma = raw.lastIndexOf(',');
    if (lastDot >= 0 && lastComma >= 0) {
      if (lastDot > lastComma) dotEvidence += 3;
      else commaEvidence += 3;
      continue;
    }
    if (lastDot >= 0 && /^-?\d+\.\d{1,2}$/.test(raw)) dotEvidence++;
    if (lastComma >= 0 && /^-?\d+,\d{1,2}$/.test(raw)) commaEvidence++;
  }
  if (dotEvidence > commaEvidence && dotEvidence > 0) return '.';
  if (commaEvidence > dotEvidence && commaEvidence > 0) return ',';
  return null;
}
