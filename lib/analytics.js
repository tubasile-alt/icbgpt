...
function buildQueryPlan(question, {
  ...
  const expenseWords = /gasto|despesa|custo|categoria|pagamento|fornecedor|funcionario|colaborador|folha|salario|imposto|marketing|locomocao/;
  ...
  const includeFinancials = financialWords.test(normalized);
  let includeAttendance = attendanceWords.test(normalized) || includeFinancials;
  if (!includeExpenses && !includeAttendance) includeExpenses = includeAttendance = true;

  const latestForSources = includeExpenses && includeAttendance
    ? [latestAttendance, latestExpenses].filter(Boolean).sort()[0]
    : includeExpenses ? latestExpenses : latestAttendance;
  ...
  const types = detectKnownLabels(normalized, knownTypes);
  ...
  if (/tipo de gasto|subtipo/.test(normalized)) expenseDimension = 'type';
  ...
  const attendanceDimension = /medico|profissional|cirurgiao/.test(normalized) ? 'professional' : null;
  ...
  if (!periods.size) {
    for (let offset = -5; offset <= 0; offset++) periods.add(addMonths(reference, offset));
  }
  ...
  return {
    ...
    groupBy,
    expenseDimension,
    expenseLabels: expenseDimension === 'category' ? categories : expenseDimension === 'type' ? types : expenseDimension === 'employee' ? expenseEmployees : [],
    attendanceDimension,
...
function queryProfessionalBreakdown(atd, plan) {
  ...
  for (const row of atd.r) {
    ...
    const key = `${unidade}|${profissional}`;
    if (!grouped.has(key)) {
      grouped.set(key, { unidade, profissional, consultas: 0, retornos: 0, cirurgias: 0 });
    }
    const target = grouped.get(key);
    target.consultas += Number(row[3]) || 0;
    target.retornos += Number(row[4]) || 0;
  ...
  return [...grouped.values()]
    .sort((a, b) => b.cirurgias - a.cirurgias || b.consultas - a.consultas)
    .slice(0, requested.size ? 30 : 10);
}
...
function formatContext(result, maxRows = 50) {
  const { plan, rows, expenseBreakdown = [], professionalBreakdown = [], meta, warnings } = result;
  ...
  if (professionalBreakdown.length) {
    lines.push('', '## ATENDIMENTO POR PROFISSIONAL (TOP 10 OU NOMES SOLICITADOS)', 'unidade|profissional|consultas|retornos|cirurgias');
    for (const row of professionalBreakdown) {
      lines.push(`${row.unidade}|${row.profissional}|${row.consultas}|${row.retornos}|${row.cirurgias}`);
    }
  }
...
function createAnalyticsEngine(embedded, parsedCurrentGastos, { now = new Date() } = {}) {
  ...
  const query = input => {
    ...
    const plan = {
      ...inferred,
      includeExpenses: typeof requested.includeExpenses === 'boolean' ? requested.includeExpenses : inferred.includeExpenses,
      includeAttendance: typeof requested.includeAttendance === 'boolean' ? requested.includeAttendance : inferred.includeAttendance,
      includeFinancials: typeof requested.includeFinancials === 'boolean' ? requested.includeFinancials : inferred.includeFinancials,
      ...
      units: Array.isArray(requested.units) && requested.units.length
        ...
        : inferred.units,
      groupBy: ['month', 'month_unit', 'unit'].includes(requested.groupBy) ? requested.groupBy : inferred.groupBy
    };
