...
function mergeConversationPlan(inferred, previous = {}) {
  ...
  const metricFields = [
    ...
    'expenseDimension', 'expenseLabels', 'attendanceDimension', 'professionalLabels'
  ];
  const metricState = {};
  if (!explicit.metrics) {
...
function formatContext(result, maxRows = 50) {
  const { plan, rows, expenseBreakdown = [], professionalBreakdown = [], executiveSummary, inheritedContext = [], meta, warnings } = result;
