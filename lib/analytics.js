...
function parseGastosCsv(text, { now = new Date(), knownUnits = [] } = {}) {
  ...
  const moneyDecimalSeparator = inferMoneyDecimalSeparator(rows.slice(1), indexes.valor);
  ...
  for (const columns of rows.slice(1)) {
    ...
    const valor = parseMoney(columns[indexes.valor], moneyDecimalSeparator);
...
function buildQueryPlan(question, {
  ...
  const executiveKpis = /\bceo\b|diretor(?:a)? executiv|(?:visao|analise|panorama|olhar|perspectiva|resumo|painel|indicadores?) executiv|visao da diretoria|para (?:a )?diretoria|como (?:a )?diretoria|nivel executiv/.test(normalized)
